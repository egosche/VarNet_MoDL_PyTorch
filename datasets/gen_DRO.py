...
def gen_DRO(data) -> tuple[
    ...
    Args:
        data: Raw acquisition data of one subject.
    ...
    mask = data['mask'].item()  # Mask for the subject
    aif = data['AIF'].item()  # Arterial Input Function
    S0 = data['S0'].item()  # Initial signal intensity
    ...
    # Parameter map not provided, use default values
    p0 = {
        'glandular': np.array(
            [
                [0, 0],
    ...
    for i in range(len(rIdx)):
        ...
        aifci_Map[rIdx[i], cIdx[i], :] = pchip_interpolate(t_1s, temp_aif, ti)
    ...
    cts = Cp * np.tile(vp[:, :, np.newaxis], (1, 1, len(ti))) + Ce * np.tile(
        ve[:, :, np.newaxis], (1, 1, len(ti))
    )
    cts = cts[:, :, logIdx]
    ...
    cts = Cp * np.tile(vp[:, :, np.newaxis], (1, 1, len(ti))) + Ce * np.tile(
        ve, (parMap.shape[0], parMap.shape[1], len(ti))
    )
    cts = cts[:, :, logIdx]
    ...
    mask_epm = (
        mask['glandular'].item() | mask['skin'].item() | mask['muscle'].item()
    )
    mask_epm = np.tile(mask_epm[:, :, np.newaxis], (1, 1, cts.shape[2])).astype(
        bool
    )
